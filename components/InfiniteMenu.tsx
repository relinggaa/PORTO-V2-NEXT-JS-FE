...
void main() {
    ...
    vAlpha = smoothstep(0.2, 0.5, normalize(centerPos).z);
    vUvs = aModelUvs;
...
class InfiniteGridMenu {
  ...
  private discGeo!: SquareGeometry;
  ...
  public SPHERE_RADIUS = 5; // Ukuran Bola (Jarak item dari tengah)
  public ITEM_SCALE = 0.7;  // Ukuran Kartu/Item
  ...
  constructor(
    ...
    this.camera.position[2] = 7 * scale;
    this.init(onInit);
  }
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (imgAspect > 1) {
          ...
          dy = y + (cellSize - drawH) / 2;
        } else {
          drawW = cellSize * imgAspect;
          dx = x + (cellSize - drawW) / 2;
        }
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.multiply(matrix, matrix, mat4.fromRotation(mat4.create(), Math.PI, [0, 1, 0]));
    ...
    <div className="relative w-full h-full ">
      ...
      {activeItem && (
        <div className={`
          ...
          ${isMoving
            ? 'opacity-0 pointer-events-none duration-[100ms] translate-y-10'
